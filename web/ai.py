...
openai.api_key = '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'
...
def create_tutorial(title, category):
    try:
        ...
        content_query = 'Write about tutorial article of minimum 900 words in the context and title of ' + title + \
                        ...
                        'output only the HTML. Do not use the <pre> tag.  Write in UTF-8 all the time. Start with h1 title.' + \
                        + title + ' At the end add list useful links for the post. Write many code examples. Do not style the HTML inline! ' + \
                        ' It is very important that you beautify and style the code inside the <code> tag.'
        print(len(content_query))
        content += get_ai_text(content_query, 3500)
